...
class Cherry_WXR_Importer extends WP_Importer {
	...
	public function __construct( $options = array() ) {
		...
		$in_progress = cdi_cache()->get( 'in_progress' );

		if ( ! $in_progress ) {

			foreach ( $empty_types as $type ) {
				cdi_cache()->update( 'post', array(), 'mapping' );
				cdi_cache()->update( 'post', array(), 'requires_remapping' );
				cdi_cache()->update( 'post', array(), 'exists' );
			}

			cdi_cache()->update( 'user_slug', array(), 'mapping' );
			cdi_cache()->update( 'term_id', array(), 'mapping' );

			cdi_cache()->update( 'in_progress', true );
