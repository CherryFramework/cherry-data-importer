		...
		foreach ( $data as $sidebar_id => $widgets ) {
			...
			if ( $sidebar_data = $this->sidebar_exists( $sidebar_id ) ) {
				...
				$sidebar_message      = esc_html__(
					'Sidebar does not exist in theme (using Inactive)', 'cherry-data-importer'
				);
			}
			...
			foreach ( $widgets as $widget_instance_id => $widget ) {
				...
				if ( ! $fail && ! isset( $available_widgets[ $id_base ] ) ) {
					...
					$widget_message      = esc_html__( 'Site does not support widget', 'cherry-data-importer' ); // explain why widget not imported
				}

				// Filter to modify settings object before conversion to array and import
				// Leave this filter here for backwards compatibility with manipulating objects (before conversion to array below)
				// Ideally the newer wie_widget_settings_array below will be used instead of this
				$widget = apply_filters( 'cherry_data_importer_widget_settings', $widget ); // object
				...
				$widget = apply_filters( 'cherry_data_importer_widget_settings_array', $widget );
				...
				if ( ! $fail && isset( $widget_instances[ $id_base ] ) ) {
					...
					foreach ( $single_widget_instances as $check_id => $check_widget ) {
						...
						if ( in_array( "$id_base-$check_id", $sidebar_widgets ) && (array) $widget == $check_widget ) {
							...
							$widget_message      = esc_html__( 'Widget already exists', 'cherry-data-importer' ); // explain why widget not imported
				...
				if ( ! $fail ) {
					...
					if ( $sidebar_available ) {
						...
						$widget_message = esc_html__( 'Imported', 'cherry-data-importer' );
					} else {
						$widget_message_type = 'warning';
						$widget_message = esc_html__( 'Imported to Inactive', 'cherry-data-importer' );
					}
				...
				$results[ $sidebar_id ]['widgets'][ $widget_instance_id ]['title'] = ! empty( $widget['title'] ) ? $widget['title'] : esc_html__( 'No Title', 'cherry-data-importer' ); // show "No Title" if widget instance is untitled
